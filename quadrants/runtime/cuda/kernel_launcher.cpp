...
KernelLauncher::Handle KernelLauncher::register_llvm_kernel(const LLVM::CompiledKernelData &compiled) {
  ...
  if (!compiled.get_handle()) {
    ...
    if (!data.per_construct_artifacts.empty()) {
      // Per-task cubin path: assemble the module from self-contained per-task artifacts (freshly built sub-modules
      // and/or cubins loaded from the on-disk per-task artifact cache) via cuLink, instead of loading the
      // whole-module PTX.
