...
LLVMCompiledKernel KernelCodeGen::compile_kernel_to_module() {
  ...
  // The per-task IR cache key is what the relocatable-cubin disk cache is keyed by (§9.D Part B), so it must be
  // available to the JIT for every compile. Cheap (one string per task, already computed for the in-memory cache
  // lookup).
